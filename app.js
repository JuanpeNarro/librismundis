...
// Quotes DOM Elements
let quotesGrid;
let quotesEmptyState;
let quotesSearchInput;
let addQuoteBtn;
let addQuoteModal;
let closeAddQuoteModal;
let cancelAddQuote;
let addQuoteForm;
let quoteDetailsModal;
let closeQuoteDetailsModal;
let editQuoteForm;
let deleteQuoteBtn;
let quotesView;
...
function saveToLocalStorage() {
    try {
        ...
        const quotesKey = currentUser ? `librismundis_user_${currentUser.id}_quotes` : 'librismundis_guest_quotes';
...
function loadFromLocalStorage() {
    try {
        ...
        vocabulary = storedVocab ? JSON.parse(storedVocab) : [];
...
function openAddQuoteModal() {
    addQuoteModal.classList.add('active');
}

function closeAddQuoteModalFunc() {
    addQuoteModal.classList.remove('active');
    addQuoteForm.reset();
}

// Forces global scope exposure
window.openQuoteDetails = function (id) {
    // DEBUG: Alert to trace flow
    // alert('Open Details Called for ID: ' + id);

    const quote = getQuote(id);
    if (!quote) {
        alert('Error: Cita no encontrada. ID: ' + id);
        console.error('Quote not found', id, quotes);
        return;
    }
    ...
    setVal('detailQuoteText', quote.text);
    // Backward compatibility: if no book/author, use source for book
    setVal('detailQuoteBook', quote.book || quote.source || '');
    setVal('detailQuoteAuthor', quote.author || '');

    setVal('detailQuoteChapter', quote.chapter || '');
    setVal('detailQuotePage', quote.page || '');
    setVal('detailQuoteContext', quote.context || '');
...
// Global robust close function for quote details
window.closeQuoteDetailsModalFunc = function () {
    const modal = document.getElementById('quoteDetailsModal');
    if (modal) {
        modal.classList.remove('active');
        modal.style.display = 'none';
    }
    currentQuoteId = null; // Clear selection
}
...
function createQuote(text, book, author, chapter, page, context) {
    return {
        id: generateId(),
        text: text.trim(),
        book: book ? book.trim() : '',
...
function addQuote(quote) {
    ...
    updateXP(5, 'Cita agregada'); // +5 XP per quote
    saveToLocalStorage();
    renderQuotes();
    renderUserStats();
}
...
function getFilteredQuotes(searchQuery = '') {
    let filtered = quotes;
    if (searchQuery) {
...
function renderQuotes(searchQuery = '') {
    const filteredQuotes = getFilteredQuotes(searchQuery);

    // Pagination
    const totalQuotes = filteredQuotes.length;
    const startIndex = (currentQuotesPage - 1) * quotesPerPage;
    const endIndex = startIndex + quotesPerPage;
    const quotesToShow = filteredQuotes.slice(startIndex, endIndex);

    renderPaginationControls(totalQuotes, quotesPerPage, currentQuotesPage, 'changeQuotesPage', 'quotesPagination');

    if (filteredQuotes.length === 0) {
        if (quotesGrid) quotesGrid.classList.add('hidden');
        if (quotesEmptyState) quotesEmptyState.classList.remove('hidden');
        const paginationContainer = document.getElementById('quotesPagination');
        if (paginationContainer) paginationContainer.innerHTML = '';
    } else {
        if (quotesGrid) quotesGrid.classList.remove('hidden');
        if (quotesEmptyState) quotesEmptyState.classList.add('hidden');

        quotesGrid.innerHTML = quotesToShow.map((quote, index) => {
            // Backward compatibility
            const bookTitle = quote.book || quote.source || 'Desconocido';
            const authorName = quote.author || '';
            const quoteNumber = totalQuotes - (startIndex + index);

            return `
            <div class="quote-card" data-id="${quote.id}" onclick="window.openQuoteDetails('${quote.id}')">
                <div class="quote-header-row" style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.5rem;">
                    <span class="quote-number" style="font-size: 1rem; color: var(--text-tertiary); font-weight: 700; opacity: 0.5;">#${quoteNumber}</span>
                </div>
                ...
                <div class="quote-footer">
                    <div class="quote-source-info">
                        <p class="quote-book">${escapeHtml(bookTitle)}</p>
                        ${authorName ? `<p class="quote-author-name">por ${escapeHtml(authorName)}</p>` : ''}
                    </div>
                </div>
                <div class="quote-meta-row">
                    <div class="quote-meta">
                        ${quote.chapter ? `<span>${escapeHtml(quote.chapter)}</span>` : ''}
                        ${quote.page ? `<span>Pág. ${quote.page}</span>` : ''}
                    </div>
                </div>
                ${quote.context ? `<div class="quote-tags">🏷️ ${escapeHtml(quote.context)}</div>` : ''}
            </div>
        `}).join('');
    }
}
...
function changeQuotesPage(page) {
    ...
    renderQuotes(quotesSearchInput ? quotesSearchInput.value : '');
}
...
function renderUserStats() {
    ...
    let progressPercentage = ((userStats.xp - currentThreshold) / (nextThreshold - currentThreshold)) * 100;
...
function migrateGuestData(userId) {
    ...
    const guestQuotes = localStorage.getItem('librismundis_guest_quotes');
    ...
    // If guest has quotes, copy to user account
    if (guestQuotes && guestQuotes !== '[]') {
        localStorage.setItem(`librismundis_user_${userId}_quotes`, guestQuotes);
        console.log('Migrated quotes from guest to user');
    }
    ...
    localStorage.removeItem('librismundis_guest_quotes');
...
function init() {
    ...
    // Quotes DOM Elements Init
    quotesGrid = document.getElementById('quotesGrid');
    quotesEmptyState = document.getElementById('quotesEmptyState');
    quotesSearchInput = document.getElementById('quotesSearchInput');
    addQuoteBtn = document.getElementById('addQuoteBtn');
    addQuoteModal = document.getElementById('addQuoteModal');
    closeAddQuoteModal = document.getElementById('closeAddQuoteModal');
    cancelAddQuote = document.getElementById('cancelAddQuote');
    addQuoteForm = document.getElementById('addQuoteForm');
    quoteDetailsModal = document.getElementById('quoteDetailsModal');
    closeQuoteDetailsModal = document.getElementById('closeQuoteDetailsModal');
    ...
    navTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            if (quotesView) quotesView.classList.add('hidden');
            ...
            if (currentView === 'library') {
                ...
                if (quotesView) quotesView.classList.remove('hidden');
                renderQuotes();
    ...
    // Delegated Quote Click Listener
    // Delegated Quote Click Listener
    // Delegated Quote Click Listener
    if (quotesGrid) {
    ...
    // Close logic for Quote Details
    if (closeQuoteDetailsModal) closeQuoteDetailsModal.onclick = window.closeQuoteDetailsModalFunc;

    // Also handle escape key for better UX
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            if (quoteDetailsModal && quoteDetailsModal.classList.contains('active')) {
                window.closeQuoteDetailsModalFunc();
            }
        }
    });

    if (addQuoteBtn) addQuoteBtn.addEventListener('click', openAddQuoteModal);
    if (closeAddQuoteModal) closeAddQuoteModal.addEventListener('click', closeAddQuoteModalFunc);
    if (cancelAddQuote) cancelAddQuote.addEventListener('click', closeAddQuoteModalFunc);

    if (addQuoteForm) {
        addQuoteForm.addEventListener('submit', (e) => {
            e.preventDefault();
            const text = document.getElementById('quoteText').value;
            const book = document.getElementById('quoteBook').value;
            const author = document.getElementById('quoteAuthor').value;
            const chapter = document.getElementById('quoteChapter').value;
            const page = document.getElementById('quotePage').value;
            const context = document.getElementById('quoteContext').value;

            const quote = createQuote(text, book, author, chapter, page, context);
            addQuote(quote);
            closeAddQuoteModalFunc();
        });
    }

    if (closeQuoteDetailsModal) {
        closeQuoteDetailsModal.addEventListener('click', () => {
            quoteDetailsModal.classList.remove('active');
        });
    }
    ...
    if (editQuoteForm) {
        editQuoteForm.addEventListener('submit', (e) => {
            ...
            const updates = {
                text: document.getElementById('detailQuoteText').value.trim(),
                book: document.getElementById('detailQuoteBook').value.trim(),
                author: document.getElementById('detailQuoteAuthor').value.trim(),
                chapter: document.getElementById('detailQuoteChapter').value.trim(),
                page: parseInt(document.getElementById('detailQuotePage').value) || null,
                context: document.getElementById('detailQuoteContext').value.trim()
            };

            updateQuote(currentQuoteId, updates);
            quoteDetailsModal.classList.remove('active');
        });
    }
    ...
    if (deleteQuoteBtn) {
        deleteQuoteBtn.addEventListener('click', () => {
            if (currentQuoteId && confirm('¿Estás seguro de que quieres eliminar esta cita?')) {
                deleteQuote(currentQuoteId);
                quoteDetailsModal.classList.remove('active');
            }
        });
    }
    ...
    renderBooks();
    renderStatistics();
    // Render Quotes if initial view is quotes (unlikely but safe)
}

// Quotes Helper Functions
window.openAddQuoteModal = function () {
    console.log('Opening Add Quote Modal (Global)');
...
window.handleAddQuote = function (e) {
    if (e) e.preventDefault();
    console.log('Handling Add Quote Submit (Global)');
    ...
    if (typeof createQuote === 'function' && typeof addQuote === 'function') {
        const quote = createQuote(text, book, author, chapter, page, context);
        addQuote(quote);
    }
...
function openQuoteDetails(id) {
    ...
    setVal('detailQuoteText', quote.text);
    setVal('detailQuoteSource', quote.source);
    setVal('detailQuoteChapter', quote.chapter || '');
    setVal('detailQuotePage', quote.page || '');
    setVal('detailQuoteContext', quote.context || '');
...
// Expose to global scope to ensure onclick works
window.openAddQuoteModal = openAddQuoteModal;
window.closeAddQuoteModalFunc = closeAddQuoteModalFunc;
window.openQuoteDetails = openQuoteDetails;

// Start the app
// Final Robust Quote Handler
document.addEventListener('DOMContentLoaded', () => {
    init();

    // Production-ready Fix for Quotes Modal
    // We attach this separately to ensure it works even if other parts of init() encounter issues
    ...
    if (btn && modal) {
        btn.onclick = (e) => {
            ...
            // Move modal to body to ensure it's not hidden by any parent containers (z-index/overflow context fix)
            if (modal.parentNode !== document.body) {
                document.body.appendChild(modal);
            }
    ...
    // Ensure Empty State button also works
    const emptyStateBtn = document.querySelector('#quotesEmptyState .btn-primary');
    if (emptyStateBtn && modal) {
        emptyStateBtn.onclick = (e) => {
            e.preventDefault();
            if (modal.parentNode !== document.body) {
                document.body.appendChild(modal);
            }
    ...
    // Robust Close Logic
    const closeBtn = document.getElementById('closeAddQuoteModal');
    ...
    if (form) {
        form.onsubmit = (e) => {
            ...
            // Use global createQuote/addQuote if available
            if (typeof createQuote === 'function' && typeof addQuote === 'function') {
                const quote = createQuote(text, book, author, chapter, page, context);
                addQuote(quote);
            } else {
                ...
                const newQuote = {
                    id: Date.now(),
                    text, book, author, chapter, page, context,
                    date: new Date().toISOString()
                };
                let q = JSON.parse(localStorage.getItem('librismundis_quotes') || '[]');
                q.push(newQuote);
                localStorage.setItem('librismundis_quotes', JSON.stringify(q));
    ...
    // Force Render Gamification
    if (typeof renderUserStats === 'function') {
        renderUserStats();
    }
});