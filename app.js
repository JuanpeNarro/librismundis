...
function renderUserStats() {
    ...
    userStatsContainer.innerHTML = `
        <div class="user-level-badge" title="${getLevelTitle(userStats.level)}">
            <span class="level-number">${userStats.level}</span>
            <span class="level-icon">🏆</span>
        </div>
        <div class="user-xp-container">
            <div class="user-xp-info">
                <span class="xp-label">Nivel ${userStats.level}: ${getLevelTitle(userStats.level)}</span>
                <span class="xp-values">${userStats.xp} / ${nextThreshold} XP</span>
            </div>
            <div class="xp-progress-bar">
                <div class="xp-progress-fill" style="width: ${progress}%"></div>
            </div>
        </div>
        <div class="streak-counter" title="Racha de días">
...
function init() {
    ...
    if (addBookForm) {
        addBookForm.addEventListener('submit', (e) => {
            ...
            const book = createBook(title, author, pages, category, language, 0, 0, comments, coverUrl);
            addBook(book);
            closeAddBookModal();
